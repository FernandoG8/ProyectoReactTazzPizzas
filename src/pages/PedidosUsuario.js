        ...
        pedidos.map((order) => (
          <div key={order.orderId} className="pedido-card">
            <div className="pedido-header">
              ...
              <span>{order.orderDate ? new Date(order.orderDate).toLocaleDateString() : 'Fecha no disponible'}</span>
            </div>
            <ul className="pedido-items">
              {order.orderItems?.map((item) => (
                <li key={item.orderItemId} className="pedido-item">
                  <span>{item.product?.productName || 'Producto'} x{item.quantity}</span>
            ...
            <div className="pedido-footer">
              ...
              <span>Método: {order.payment?.pgName || order.payment?.paymentMethod}</span>
              <span>Estado Pago: {order.payment?.pgStatus || 'N/A'}</span>
